...
const Estoque: React.FC = () => {
  ...
  const [filtroStatus, setFiltroStatus] = useState<
    "todos" | "ativos" | "vencidos" | "proximos"
  >("todos");
  ...
  const getStatusColor = (insumo: Insumo) => {
    ...
    if (isProximoVencer(insumo.data_validade))
      ...
      {visualizacao === "cards" ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {insumosFiltrados.map((insumo) => (
            <div
              key={insumo.id}
              className={`bg-white rounded-lg shadow-sm overflow-hidden border-l-4 ${getStatusColor(
                insumo
              )}`}
            >
              <div className="p-4">
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        Tem certeza que deseja desativar este insumo? Esta ação
