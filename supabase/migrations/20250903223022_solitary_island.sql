/*
  # Corrigir Erro de Sintaxe na Migration Light Haze

  1. Problema Identificado
    - Migration 20250903221546_light_haze.sql usa `current_time` como nome de variável
    - `current_time` é palavra reservada no PostgreSQL
    - Erro: syntax error at or near "current_time"

  2. Solução
    - Recriar todas as funções com variável renomeada para `current_timestamp_epoch`
    - Dropar funções existentes primeiro para evitar conflitos
  ...
  3. Funções Corrigidas
    ...
    - debug_stripe_configuration
    - stripe_user_subscriptions view
*/

-- Drop existing functions with all possible signatures to avoid conflicts
DROP FUNCTION IF EXISTS user_has_active_subscription(uuid);
DROP FUNCTION IF EXISTS get_user_subscription_details(uuid);
DROP FUNCTION IF EXISTS handle_subscription_success(text, text, text, text, integer, integer);
DROP FUNCTION IF EXISTS debug_stripe_configuration();
...
DO $$
BEGIN
  RAISE NOTICE 'Erro de sintaxe da migration light_haze corrigido com sucesso!';
  RAISE NOTICE 'Funções Stripe recriadas com variáveis corretas';
