/*
  # Fix Stripe Price ID and Syntax Error - Final Solution

  1. Problems Fixed
    - Price ID 'price_1SzW0KB4if3rE1yX3gGCzDaQ' does not exist in Stripe
    - Syntax error with 'current_time' reserved keyword
    - View dependency preventing function drops

  2. Solutions
    - Drop view first to remove dependencies
    - Use CASCADE to drop all dependent objects
    - Update Price IDs to use existing valid ones from your Stripe account
    - Fix all syntax errors with current_timestamp_epoch

  3. Valid Price IDs
    - Use your existing Price IDs that are already working
    - Remove the non-existent test Price ID
    - Map to your real products
*/
...
BEGIN
  RETURN CASE price_id
    -- Your existing valid Price IDs only
...
DO $$
BEGIN
  RAISE NOTICE 'Price ID and syntax errors fixed successfully!';
  ...
  RAISE NOTICE 'All dependencies resolved with CASCADE!';
END $$;