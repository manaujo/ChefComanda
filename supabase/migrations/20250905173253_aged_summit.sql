/*
  # Corrigir Herança de Plano para Funcionários - Solução Final
  ...
  1. Problema Identificado
    ...
    - Dependências de views impedem DROP das funções

  2. Solução Final
    - Usar CASCADE para dropar todas as dependências
    - Recriar todas as funções com parâmetros prefixados
    - Implementar herança correta de planos
    - Atualizar views com nova lógica

  3. Funcionalidades
    - Funcionários herdam acesso do plano do admin automaticamente
    - Verificação hierárquica de assinatura
    - Interface adaptada para funcionários
    - Debug completo para troubleshooting
*/

-- Drop all dependent views and functions with CASCADE
DROP VIEW IF EXISTS stripe_user_subscriptions CASCADE;
DROP VIEW IF EXISTS employee_inherited_subscriptions CASCADE;
DROP FUNCTION IF EXISTS user_has_system_access(uuid) CASCADE;
DROP FUNCTION IF EXISTS employee_has_inherited_subscription(uuid) CASCADE;
DROP FUNCTION IF EXISTS get_effective_subscription_details(uuid) CASCADE;
DROP FUNCTION IF EXISTS debug_subscription_inheritance(uuid) CASCADE;
