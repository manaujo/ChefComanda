/*
  # Fix Stripe View Already Exists Error

  1. Problem Fixed
    - View "stripe_user_invoices" already exists
    - Need to use DROP VIEW IF EXISTS before recreating
    - Also fix the Price ID that doesn't exist in Stripe

  2. Solution
    - Drop all existing views with CASCADE
    - Remove non-existent Price ID from configuration
    - Recreate everything with only valid Price IDs
    - Use IF EXISTS to prevent future conflicts

  3. Valid Price IDs Only
    - price_1RucPuB4if3rE1yXh76pGzs7 (Plano Mensal)
    - price_1RvfteB4if3rE1yXvpuv438F (Plano Trimestral)
    - price_1RucR4B4if3rE1yXEFat9ZXL (Plano Anual)
*/

-- Drop all existing views and functions with CASCADE to remove dependencies
...
-- Update get_plan_name_from_price_id function with ONLY your existing valid Price IDs
...
BEGIN
  RETURN CASE price_id
    -- Your existing valid Price IDs only (removed the non-existent test price)
...
DO $$
BEGIN
  RAISE NOTICE 'Stripe view exists error fixed successfully!';
  RAISE NOTICE 'Removed non-existent Price ID: price_1SzW0KB4if3rE1yX3gGCzDaQ';
  RAISE NOTICE 'Fixed current_time syntax error with current_timestamp_epoch';
  RAISE NOTICE 'Used CASCADE to resolve all dependencies';
  RAISE NOTICE 'Valid Price IDs configured:';
  ...
  RAISE NOTICE 'All views and functions recreated successfully!';
END $$;