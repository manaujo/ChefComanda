/*
  # Update Stripe Configuration for Teste Product Only

  1. Price ID Mapping Function
    - Update with real Price ID for Teste product
    - Remove all other products from mapping

  2. Product Configured
    - Teste: price_1S2w0KB4if3rE1yX3gGCzDaQ (R$ 1,00/ano)
...
-- Update the plan name mapping function with only the Teste product
...
-- Update user_has_active_subscription function to work with the Teste product
...
DO $$
BEGIN
  RAISE NOTICE 'Stripe configuration updated for Teste product only!';
  RAISE NOTICE 'Product configured:';
  RAISE NOTICE '- Teste: price_1S2w0KB4if3rE1yX3gGCzDaQ (R$ 1,00/ano)';
  RAISE NOTICE 'Product ID: prod_SytnLTTRUGsctD';
  RAISE NOTICE 'Mode: subscription';
  RAISE NOTICE 'System ready for Teste product only!';
END $$;