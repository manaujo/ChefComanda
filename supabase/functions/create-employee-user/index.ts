...
Deno.serve(async (req: Request) => {
  try {
    ...
    // Check if CPF already exists in employees table
    const { data: existingEmployee, error: cpfError } = await supabaseAdmin
      .from("employees")
      .select("id")
      .eq("cpf", cpf)
      .maybeSingle();
    ...
    if (cpfError && cpfError.code !== "PGRST116") {
      ...
      return new Response(JSON.stringify({ error: "Erro ao verificar CPF" }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" }
      });
    }
    ...
    if (existingEmployee) {
      return new Response(JSON.stringify({ error: "CPF já está cadastrado" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" }
      });
    }
