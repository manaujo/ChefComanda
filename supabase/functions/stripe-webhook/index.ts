...
console.log('🔐 Webhook Secret configured:', stripeWebhookSecret ? 'YES' : 'NO');
...
async function handleEvent(event: Stripe.Event) {
  ...
  if (!stripeData) {
    console.warn('⚠️ No data in event');
    return;
  }

  if (!('customer' in stripeData)) {
    console.warn('⚠️ No customer in event data');
    return;
  }

  // for one time payments, we only listen for the checkout.session.completed event
  if (event.type === 'payment_intent.succeeded' && event.data.object.invoice === null) {
    console.log('ℹ️ Skipping one-time payment_intent.succeeded');
    return;
  }
